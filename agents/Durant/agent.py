...
#     - Keep a real lookahead search (3–4 ply rollout with enemy policy).
#     - Use a dense, well-structured evaluation:
#         * egg diff
#         * BFS-based reachable-region / territory advantage
#         * mobility (local branching + legal moves)
#         * trap risk
...
#                 # Hard timeout: fall back to a fast greedy move
...
#         depth = self._adaptive_depth(board, legal_moves, remaining)

#         budget = max(remaining - self.safety_buffer, self.min_budget)
#         self._search_deadline = time.perf_counter() + max(budget, self.min_budget)

#         ordered = self._order_moves(board, legal_moves)

#         # --------------------------------------------------------------
#         # Anti-self-blocking filter (root-only):
#         # Reject moves that leave us with tiny reachable region while
#         # opponent's reachable region remains large.
#         # --------------------------------------------------------------
#         my_loc = board.chicken_player.get_location()
#         enemy_loc = board.chicken_enemy.get_location()
#         base_my_region = self._reachable_region(board, my_loc)
#         base_opp_region = self._reachable_region(board, enemy_loc)

#         safe_moves: List[Tuple[Direction, MoveType]] = []
#         catastrophic_moves: List[Tuple[Direction, MoveType]] = []

#         # Dynamic thresholds: relative to current regions so we don't
#         # over-filter when we're already in a cramped position.
#         tiny_threshold = max(3, base_my_region // 4)

#         for mv in ordered:
#             my_r, opp_r = self._reachable_regions_after_move(board, mv)
#             # Very bad if our region collapses while theirs stays big.
#             if my_r <= tiny_threshold and (opp_r - my_r) >= 6:
#                 catastrophic_moves.append(mv)
#             else:
#                 safe_moves.append(mv)

#         candidates = safe_moves if safe_moves else ordered

#         best_move = candidates[0]
#         best_score = -INF
#         evaluated = 0

#         limit = min(len(candidates), self._branch_limit(depth))
#         for move in candidates[:limit]:
#             try:
#                 score = self._rollout_after_move(board, move, depth)
#             except SearchTimeout:
#                 # If we've evaluated at least one move, keep the best so far.
#                 if evaluated > 0:
#                     break
#                 # No moves evaluated yet -> let caller fall back.
#                 raise
#             evaluated += 1
#             if score > best_score:
#                 best_score = score
...
#     ) -> int:
#         """Adjust search depth based on phase, branching and remaining time."""
#         depth = 3
...
#         # If we are low on time, cap depth aggressively.
...
#         # Global empty-count-driven endgame + reachable-region mobility
#         empty_cells = self._count_empty_cells(board)
#         is_endgame = self.phase == "endgame" or empty_cells <= 12

#         region_self = self._reachable_region(
#             board, board.chicken_player.get_location()
#         )
#         region_opp = self._reachable_region(
#             board, board.chicken_enemy.get_location()
#         )
...
#         if is_endgame:
#             # Egg race + territory sealing
#             egg_weight = 210.0
#             mobility_weight = 4.5
#             terr_weight = 3.0
#             region_weight = 4.0

#         score = egg_weight * egg_diff
#         score += terr_weight * (terr_self - terr_opp)
#         score += mobility_weight * (mobility_self - mobility_opp)
#         score += region_weight * (region_self - region_opp)
#         score -= 40.0 * risk_here
#         score += 10.0 * choke

#         # Explicit extra penalty for self-blocked positions:
#         # our region tiny while theirs remains comfortably large.
#         if region_self <= 3 and region_opp >= region_self + 5:
#             score -= 30.0

#         # In endgame, explicitly like closing out remaining empty cells;
#         # earlier, still keep a mild "time pressure" from turns_left.
#         if is_endgame:
#             score -= 2.0 * empty_cells
#         else:
...
#         # Reachable-region baselines for delta-area heuristics
#         my_region_before = self._reachable_region(board, cur)
#         opp_region_before = self._reachable_region(board, board.chicken_enemy.get_location())
...
#             # Hard push for eggs in the true endgame egg race
#             if is_endgame:
#                 base += 25.0
#                 if empty_cells <= 8:
#                     base += 25.0
#                 if egg_diff <= 0:
#                     # Extra urgency when we are tied or behind in eggs
#                     base += 10.0 * (-egg_diff + 1)

#             return base

#         if mt == MoveType.TURD:
#             # Base local choke / corridor heuristic
...
#             # Purposeful turd scoring: delta reachable-area heuristic
#             child = board.get_copy()
#             if child.apply_move(dir_, mt):
#                 my_region_after = self._reachable_region(
#                     child, child.chicken_player.get_location()
#                 )
#                 opp_region_after = self._reachable_region(
#                     child, child.chicken_enemy.get_location()
#                 )

#                 adv_before = my_region_before - opp_region_before
#                 adv_after = my_region_after - opp_region_after
#                 delta_adv = adv_after - adv_before

#                 # Reward turds that hurt their region more than ours.
#                 base += 3.0 * delta_adv

#                 # Strong penalty for self-blocking turds.
#                 tiny_threshold = max(3, my_region_before // 3)
#                 if my_region_after <= tiny_threshold and (opp_region_after - my_region_after) >= 4:
#                     base -= 20.0
...
#         # In endgame, discourage wandering when we *could* be egging soon
#         if is_endgame and board.can_lay_egg() and self._is_my_parity(cur):
#             base -= 8.0
...
#         """
#         Phase is now also driven by how many empty cells remain, not just turns.

#         - opening: first few turns
#         - midgame: normal play
#         - endgame: when board is nearly full OR few turns remain
#         """
#         empty_cells = self._count_empty_cells(board)

#         if board.turn_count < self.OPENING_TURNS:
#             self.phase = "opening"
#         elif empty_cells <= 12 or board.turns_left_player <= self.LATE_GAME_TURNS:
#             self.phase = "endgame"
#         else:
#             self.phase = "midgame"
...
#         Uses the precomputed lane direction so it stays correct even if we
#         don't spawn exactly on column 0 / last column.
#         """
#         forward = self._lane_dir
#         script: List[Direction] = [forward, forward]
...
#         """
#         Count how many cells on the board are still empty / traversable.

#         This drives the "true" endgame switch, so Durant knows when
#         we're in a pure egg-race / territory-closure phase.
#         """
#         cnt = 0
#         for y in range(self.size):
#             for x in range(self.size):
...
#     def _reachable_region(self, board: Board, start: Coord) -> int:
#         """
#         Flood-fill from `start` over non-blocked cells.

#         This is a global mobility/territory measure: how big is *my* room
#         to maneuver vs the enemy's?
#         """
...
#     def _reachable_regions_after_move(
#         self, board: Board, move: Tuple[Direction, MoveType]
#     ) -> Tuple[int, int]:
#         """
#         Simulate a move once and compute (myReach, oppReach) on the
#         resulting board. Used for anti-self-blocking filtering.
#         """
#         child = board.get_copy()
#         if not child.apply_move(*move):
#             # Shouldn't happen for legal moves, but be safe.
#             my_loc = board.chicken_player.get_location()
#             enemy_loc = board.chicken_enemy.get_location()
#             return (
#                 self._reachable_region(board, my_loc),
#                 self._reachable_region(board, enemy_loc),
#             )
#         my_loc = child.chicken_player.get_location()
#         enemy_loc = child.chicken_enemy.get_location()
#         my_region = self._reachable_region(child, my_loc)
#         opp_region = self._reachable_region(child, enemy_loc)
#         return my_region, opp_region
...
#         """
#         Approximate distance along our primary lane toward the far edge.

#         Uses self._lane_dir so it stays consistent with the opening script.
#         """
#         if self._lane_dir in (Direction.RIGHT, Direction.LEFT):
#             target_x = self.size - 1 if self._lane_dir == Direction.RIGHT else 0
#             return abs(loc[0] - target_x)
#         # Vertical lane fallback (shouldn't usually trigger, but safe)
#         target_y = self.size - 1 if self._lane_dir == Direction.DOWN else 0
#         return abs(loc[1] - target_y)
...
#         # If they spawn on the left half, target the right edge; otherwise target left.
#         target_x = self.size - 1 if sx <= self.size // 2 else 0
#         return (target_x, sy)
...
#             else:
#                 # Reward moves that break out of tiny oscillation basins
#                 return -12.0
#         if n >= 4:
#             if (
#                 recent[-1] == recent[-3]
...
class PlayerAgent:
    ...
    def __init__(self, board: Board, time_left: Callable[[], float]):
        ...
        # Egg race state
        self.egg_deficit_mode: bool = False
        self.egg_deficit: int = 0
    ...
    def play(
        ...
        # Clear any per-board caches from previous turns (board mutates over time).
        if hasattr(board, "_durant_empty_count"):
            delattr(board, "_durant_empty_count")
        if hasattr(board, "_durant_rr_cache"):
            delattr(board, "_durant_rr_cache")
        ...
        # 7. Egg-race state (used by scoring to go into "panic egg mode")
        eggs_self = board.chicken_player.get_eggs_laid()
        eggs_opp = board.chicken_enemy.get_eggs_laid()
        self.egg_deficit = eggs_opp - eggs_self
        self.egg_deficit_mode = (
            self.phase != "opening" and self.egg_deficit >= 2
        )
        ...
        # 10. Loop / stagnation bookkeeping
        if not self.recent_positions or self.recent_positions[-1] != cur_loc:
            self.recent_positions.append(cur_loc)
    ...
    def _static_eval(self, board: Board) -> float:
        ...
        risk_here = self._risk_at(board.chicken_player.get_location())
        enemy_risk_here = self._risk_at(board.chicken_enemy.get_location())
        adj_risk = self._adjacent_risk(board.chicken_player.get_location())
        choke = self._enemy_choke_bonus(board, board.chicken_player.get_location())
        ...
        score -= 40.0 * risk_here
        score -= 8.0 * adj_risk
        score += 6.0 * enemy_risk_here
        score += 10.0 * choke

        # Being behind on eggs late is catastrophic: amplify that pain.
        if is_endgame and egg_diff < 0:
            score += 30.0 * egg_diff  # egg_diff < 0, so this subtracts more

        # Mobility collapse: punish states where we are nearly stuck.
        if mobility_self <= 2 and mobility_opp >= mobility_self + 4:
            score -= 25.0
    ...
    def _score_move(self, board: Board, move: Tuple[Direction, MoveType]) -> float:
        ...
        # Reachable-region baselines for delta-area heuristics
        my_region_before = self._reachable_region(board, cur)
        opp_region_before = self._reachable_region(
            board, board.chicken_enemy.get_location()
        )
        ...
        if mt == MoveType.EGG:
            ...
            base -= 25.0 * max(0.0, risk_nxt - 0.5)
            base -= 3.0 * dist_center_cur
            # Reward closing egg gap
            base += 8.0 * max(0, -egg_diff)
            if self._is_my_parity(cur):
                base += 8.0
            base += 2.5 * self._novelty_bonus(cur)

            # When we're in egg-deficit "panic mode", eggs get a big premium.
            if self.egg_deficit_mode:
                base += 20.0 + 6.0 * float(self.egg_deficit)
            ...
            if is_endgame:
                ...
                if egg_diff <= 0:
                    # Extra urgency when we are tied or behind in eggs
                    base += 12.0 * (-egg_diff + 1)

            return base

        # Turd-specific scoring -------------------------------------------------
        if mt == MoveType.TURD:
            # Conservative default: turds are only good when they really
            # damage enemy space without self-blocking.
            block = self._turd_block_value(board, cur)
            base = -6.0
            base += 6.0 * block
            base -= 18.0 * risk_cur
            ...
            if child.apply_move(dir_, mt):
                ...
                adv_before = float(my_region_before - opp_region_before)
                adv_after = float(my_region_after - opp_region_after)
                delta_adv = adv_after - adv_before

                opp_region_shrink = max(0, opp_region_before - opp_region_after)
                my_region_shrink = max(0, my_region_before - my_region_after)

                # Reward turds that shrink enemy space more than ours.
                base += 3.5 * max(0.0, float(opp_region_shrink - my_region_shrink))
                base += 2.0 * delta_adv

                # Strong penalty for self-blocking turds.
                tiny_threshold = max(3, my_region_before // 3)
                if (
                    my_region_after <= tiny_threshold
                    and (opp_region_after - my_region_after) >= 4
                ):
                    base -= 30.0

                # Big bonus if we genuinely trap them into a tiny pocket.
                if opp_region_after <= 3 and my_region_after >= opp_region_after + 4:
                    base += 18.0

            # Be extra stingy with early-game turds and while behind on eggs.
            if board.turn_count < self.OPENING_TURNS + 4:
                base -= 6.0
            if egg_diff < 0 or self.egg_deficit_mode:
                base -= 8.0
        ...
        stuck_pressure = min(1.0, float(self.stagnation_count) / 4.0)
        frontier_weight = (
            7.0 if self.phase == "opening" else 6.0
        ) + 1.5 * stuck_pressure
        base += 2.0 * novelty
        base += frontier_weight * frontier_step
        ...
        terr_weight = (
            0.45
            if self.phase == "opening"
            else (0.35 if self.phase == "midgame" else 0.25)
        )
        delta_weight = (
            0.6
            if self.phase == "opening"
            else (0.4 if self.phase == "midgame" else 0.25)
        )
        risk_scale = 1.0 / (1.0 + 2.0 * max(0.0, risk_nxt - 0.2))
        terr_contrib = risk_scale * (
            terr_weight * terr_adv_clamped
            + delta_weight * max(0.0, terr_delta_clamped)
        )
        terr_contrib = self._clamp(terr_contrib, -6.0, 6.0)
        base += terr_contrib
        ...
        if self.phase != "opening":
            ...
            nearest_nxt = self._nearest_egg_distance(board, nxt, limit=6)
            if nearest_nxt < nearest_cur:
                step_gain = float(nearest_cur - nearest_nxt)
                mult = 4.0
                if is_endgame or self.egg_deficit_mode:
                    mult = 7.0
                base += mult * step_gain
        ...
        if (
            ...
            and branching <= 1
            and terr_delta <= 0
            and not board.can_lay_egg_at_loc(nxt)
        ):
            base -= 6.0
    ...
    def _enemy_cutoff_bonus(self, board: Board, loc: Coord) -> float:
        ...
        if pred_dir is not None:
            pred_enemy_next = loc_after_direction(enemy_loc, pred_dir)
            if (
                self._manhattan(loc, pred_enemy_next)
                < self._manhattan(my_loc, pred_enemy_next)
            ):
                bonus += 3.0
        return bonus
    ...
    def _count_empty_cells(self, board: Board) -> int:
        ...
        we're in a pure egg-race / territory-closure phase.
        """
        cached = getattr(board, "_durant_empty_count", None)
    ...
    def _reachable_region(self, board: Board, start: Coord) -> int:
        ...
        cache = getattr(board, "_durant_rr_cache", None)
        if cache is None:
            cache = {}
            board._durant_rr_cache = cache
    ...
    def _adjacent_risk(self, loc: Coord) -> float:
        """
        Average trap/turd risk over the four von-Neumann neighbors.

        This lets evaluation care about stepping into "risk basins", not just
        the exact tile we're on.
        """
        total = 0.0
        cnt = 0
        for d in Direction:
